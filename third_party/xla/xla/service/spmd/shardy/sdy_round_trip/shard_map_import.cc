...
class SdyRoundTripShardMapImportPass
 ...
 private:
  void runOnOperation() final {
    ...
    llvm::DenseSet<StringRef> manualComputationCalleeNamesToClone;
    module->walk([&](CallOp op) {
      if (!op.getCallee().contains(kManualComputationFuncName)) {
        return;
      }
      if (manualComputationCalleeNamesToClone.insert(op.getCallee()).second) {
        return;
      }
    ...
    for (mlir::CallGraphNode* node : llvm::reverse(rpo)) {
      ...
      node->getCallableRegion()->walk([&](CallOp callOp) {
        ...
        if (mlir::failed(rewriteManualComputation(
                callOp, rewriter, symbolTable, manualComputationCalleeNames))) {
          callOp.emitError("failed to rewrite func.call to manual computation");
          success = false;
